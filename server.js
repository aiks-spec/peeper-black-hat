...
const recentVisitors = new Map(); // Track recent visitors to prevent duplicate counts
const VISITOR_DEBOUNCE_MS = 5000; // 5 seconds debounce per IP
...
app.use((req, res, next) => {
    try {
        ...
        // Check if we've already tracked this IP recently
        const now = Date.now();
        const lastVisit = recentVisitors.get(ip);
        
        if (lastVisit && (now - lastVisit) < VISITOR_DEBOUNCE_MS) {
            console.log('🔍 IP already tracked recently, skipping:', ip);
            return next();
        }
        ...
        if (recentVisitors.size > 1000) {
            const cutoff = now - VISITOR_DEBOUNCE_MS;
...
async function waitForFastAPI() {
    const maxRetries = 10; // Reduced retries
    for (let i = 0; i < maxRetries; i++) {
        try {
            ...
            if (i === maxRetries - 1) {
                console.log('❌ FastAPI backend not available after 10 attempts');
                console.log('⚠️ Starting in standalone mode (OSINT tools will use direct execution)');
                return false;
            }
...
app.get('/api/scan/email', async (req, res) => {
    try {
        ...
        console.log('🔄 Falling back to direct tool execution...');
        
        // Fallback to direct tool execution
        try {
            const results = await runToolIfAvailable('holehe', [value]);
            res.json({ 
                email: value,
                holehe: results,
                message: 'Executed via fallback mode'
            });
        } catch (fallbackError) {
            res.status(500).json({ error: 'Scan failed', details: fallbackError.message });
        }
    }
});
...
app.post('/api/run-osint-script', async (req, res) => {
    ...
    res.json({
        ...
        console.log(`❌ OSINT script failed:`, error.message);
        res.status(500).json({
            success: false,
            error: error.message,
            message: 'OSINT script execution failed'
        });
    }
});
